...
export const getCanvasState = async (req: Request, res: Response) => {
    try {
        ...
        const redisKey = `canvas:${meetingId}`;
        ...
        const dbCanvas = await BrainstormingUpdate.findOne({ meetingId: actualMeetingId, toolType: "canvas" });

        if (dbCanvas && dbCanvas.dataState) {
            if (redisClient.isOpen) {
                await redisClient.setEx(redisKey, 86400, JSON.stringify(dbCanvas.dataState));
            }

            return res.status(200).json({
                source: "mongodb",
                state: dbCanvas.dataState,
            });
        }
        ...
        return res.status(500).json({ error: "Failed to fetch canvas state" });
    }
};

export const updateCanvasState = async (req: Request, res: Response) => {
    try {
        await connect();
        ...
        if (action === "save_to_db") {
            ...
            await BrainstormingUpdate.findOneAndUpdate(
                { meetingId: actualMeetingId, toolType: "canvas" },
                {
            ...
            return res.status(200).json({
                message: "Canvas saved to database successfully",
                success: true
            });
        }
        ...
        if (state) {
            ...
            return res.status(200).json({
                message: "Canvas updated in cache",
                success: true
            });
        }
...
export const deleteCanvasCache = async (req: Request, res: Response) => {
    try {
        const meetingId = req.query.meetingId as string;
        ...
        if (redisClient.isOpen) {
            await redisClient.del(redisKey);
            return res.status(200).json({
                message: "Canvas cleaned from Redis memory",
                success: true
            });
        }
        ...
        console.error("Error deleting canvas from Redis:", error);
