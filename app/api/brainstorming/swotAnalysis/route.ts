...
export const getSwotState = async (req: Request, res: Response) => {
    try {
        await connect();
        ...
        const dbSwot = await BrainstormingUpdate.findOne({ meetingId: actualMeetingId, toolType: "swot" });

        if (dbSwot && dbSwot.dataState) {
            if (redisClient.isOpen) {
                await redisClient.setEx(redisKey, 86400, JSON.stringify(dbSwot.dataState));
            }

            return res.status(200).json({
                source: "mongodb",
                state: dbSwot.dataState,
            });
        }
        ...
        console.error("Error fetching swot state:", error);
        return res.status(500).json({ error: "Failed to fetch swot state" });
    }
};

export const updateSwotState = async (req: Request, res: Response) => {
    try {
        await connect();
        ...
        if (action === "save_to_db") {
            ...
            await BrainstormingUpdate.findOneAndUpdate(
                { meetingId: actualMeetingId, toolType: "swot" },
                {
            ...
            return res.status(200).json({
                message: "SWOT saved to database successfully",
                success: true
            });
        }
        ...
        return res.status(500).json({ error: "Failed to update swot" });
    }
};

export const deleteSwotCache = async (req: Request, res: Response) => {
    try {
        const meetingId = req.query.meetingId as string;
        ...
        if (redisClient.isOpen) {
            await redisClient.del(redisKey);
            return res.status(200).json({
                message: "SWOT cleaned from Redis memory",
                success: true
            });
        }
        ...
        console.error("Error deleting swot from Redis:", error);
